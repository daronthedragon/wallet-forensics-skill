...
/**
 * Weighted-average cost basis, with value inferred from each trade's
...
function computePositions(result, nativePriceByDay) {
  ...
  const ordered = [...result.txs].sort((a, b) => a.ts - b.ts);
  ...
  for (const tx of ordered) {
    ...
    if (!anchor && nativePrice !== undefined) {
      for (const t of tx.transfers) {
        if (t.asset !== NATIVE) continue;
        const usd = (Math.abs(Number(t.amount)) / 10 ** t.decimals) * nativePrice;
        if (usd > 0) {
          anchor = {
            usd,
            legs: tx.transfers.filter((x) => x.asset !== NATIVE && !isStable(x.asset) && x.amount !== 0n).length,
          };
          break;
        }
      }
  ...
  const haveBalances = result.balances.length > 0;
  const byAsset = new Map(result.balances.map((b) => [b.asset, b]));
...
async function main() {
  ...
  for (const { chain, address } of targets) {
    ...
    const { positions, unvalued } = computePositions(raw, nativeByDay);
